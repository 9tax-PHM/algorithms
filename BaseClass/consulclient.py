...
class ConsulClient():
    ...
    def dregister(self, service_id):
        url='http://'+self.host+':'+str(self.port)+'/v1/agent/service/deregister/'
        requests.put(url + service_id)
    ...
    def getService(self,name):#负债均衡获取服务实例
        ...
        for dc in dcset:
            if self.token:
                ...
                url='http://'+self.host+':'+self.port+'/v1/health/service/'+name+'?dc='+''+'&token='
    ...
    def getServices(self):
        data = self.consul.agent.services()
        print(data)
        # keys = {}
        # for k in (data):
        #     print(k)
        ...
        #     # print(data[k]['ID'])
        #     print(data[k]['Tags'])
        #     keys.update({data[k]['Service']: data[k]['Tags'][0]})
        # return keys